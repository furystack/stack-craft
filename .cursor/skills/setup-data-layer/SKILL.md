...
Add an entity end-to-end through the FuryStack data stack: model → physical store → throw-by-default token wiring → DataSet with auth/hooks → entity sync. Read `.cursor/rules/REST_SERVICE.mdc`, `.cursor/rules/CODE_STYLE.mdc`, and `.cursor/rules/TYPESCRIPT_GUIDELINES.mdc` before authoring; this skill owns the workflow.

> **StackCraft layout note:** Stores split by module — define stores and DataSets in `service/src/app-models/<module>/setup-<module>-store.ts`. Entity sync registration lives in `service/src/setup-entity-sync.ts`. The skill's generic `service/src/setup-store.ts` reference should be translated to the per-module path.
