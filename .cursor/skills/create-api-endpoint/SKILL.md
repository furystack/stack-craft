...
Add a REST endpoint end-to-end. Touches `common/`, `service/`, possibly `frontend/`. Read `.cursor/rules/REST_SERVICE.mdc`, `.cursor/rules/CODE_STYLE.mdc`, and `.cursor/rules/TYPESCRIPT_GUIDELINES.mdc` before authoring; this skill owns the workflow.

> **StackCraft layout note:** APIs split by module — type contracts in `common/src/apis/<module>.ts`, endpoints in `service/src/app-models/<module>/setup-<module>-rest-api.ts`. The skill's generic `common/src/<api-name>.ts` and `service/src/setup-rest-api.ts` references should be translated to the per-module paths. `service/src/service.ts` composes all per-module setup calls.
