...
describe('useRequestLogger', () => {
  ...
  let loggerCollection: ReturnType<typeof getLogger>

  beforeEach(() => {
    injector = new Injector()
    useLogging(injector, VerboseConsoleLogger)
    loggerCollection = getLogger(injector)
  })
  ...
  it('should log at verbose level for 2xx status codes', async () => {
  ...
  it('should log at warning level for 4xx status codes', async () => {
  ...
  it('should log at error level for 5xx status codes', async () => {
  ...
  it('should use UNKNOWN for missing req.method', async () => {
    const verboseSpy = vi.spyOn(loggerCollection, 'verbose')
    const middleware = useRequestLogger(injector)
  ...
  it('should use / for missing req.url', async () => {
    const verboseSpy = vi.spyOn(loggerCollection, 'verbose')
    const middleware = useRequestLogger(injector)
  ...
  it('should include method, url, statusCode and duration in the log message', async () => {
    const verboseSpy = vi.spyOn(loggerCollection, 'verbose')
    const middleware = useRequestLogger(injector)
