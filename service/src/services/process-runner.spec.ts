...
describe('ProcessRunner', () => {
  ...
  let mockLogStorage: { addEntry: ReturnType<typeof vi.fn> }
  ...
  beforeEach(() => {
    ...
    runner = injector.getInstance(ProcessRunner)
  })
  ...
  describe('addLogLine', () => {
    ...
    it('should schedule a flush timer on first buffered line', async () => {
      runner.processes.set('svc-1', {
        serviceId: 'svc-1',
        process: {} as ChildProcess,
        purpose: 'run',
  ...
  describe('flushLogBuffer', () => {
    it('should flush buffered entries to logStorage', async () => {
      runner.processes.set('svc-1', {
        serviceId: 'svc-1',
        process: {} as ChildProcess,
        purpose: 'run',
  ...
  describe('[Symbol.asyncDispose]', () => {
    it('should clear flush timer and flush remaining log lines', async () => {
      runner.processes.set('svc-1', {
        serviceId: 'svc-1',
        process: {} as ChildProcess,
        purpose: 'run',
    ...
    it('should be safe to call when no timer is active', async () => {
      await expect(runner[Symbol.asyncDispose]()).resolves.toBeUndefined()
    })
  })

  describe('spawnCommand', () => {
    it('should call spawn with correct shell arguments', async () => {
      const originalPlatform = process.platform
      Object.defineProperty(process, 'platform', { value: 'linux', writable: true })
    ...
    it('should merge extraEnv into the environment', async () => {
      const originalPlatform = process.platform
      Object.defineProperty(process, 'platform', { value: 'linux', writable: true })
    ...
    it('should use cmd.exe on Windows', async () => {
      ...
      expect(spawn).toHaveBeenCalledWith('cmd.exe', ['/c', 'echo hello'], expect.objectContaining({ cwd: 'C:\\temp' }))
