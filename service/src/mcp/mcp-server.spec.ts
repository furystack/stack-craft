...
describe('McpSessionManager', () => {
  ...
  it('should register a transport and track it', () => {
    ...
    manager.register('session-1', transport)
  ...
  it('should remove transport on close callback', () => {
    ...
    manager.register('session-2', transport)
    expect(manager.size).toBe(1)
  ...
  it('should sweep expired sessions', () => {
    ...
    manager.register('old-session', transport1)

    vi.advanceTimersByTime(31 * 60 * 1000)

    manager.register('new-session', transport2)
  ...
  it('should close all transports on dispose', () => {
    const close1 = vi.fn()
    const close2 = vi.fn()
    manager.register('s1', { close: close1, onclose: null } as never)
    manager.register('s2', { close: close2, onclose: null } as never)
  ...
  it('should track multiple sessions independently', () => {
    ...
    manager.register('a', t1)
    manager.register('b', t2)
    manager.register('c', t3)
