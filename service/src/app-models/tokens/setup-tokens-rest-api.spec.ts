...
describe('Token CRUD operations', () => {
  ...
  describe('GetTokensAction', () => {
    it('should return only tokens belonging to the current user', async () => {
      mockedGetCurrentUser.mockResolvedValue({ username: 'admin', roles: ['admin'] })
  ...
  describe('DeleteTokenAction', () => {
    it('should delete a token owned by the current user', async () => {
      mockedGetCurrentUser.mockResolvedValue({ username: 'admin', roles: ['admin'] })
    ...
    it("should throw 404 when deleting another user's token", async () => {
      mockedGetCurrentUser.mockResolvedValue({ username: 'admin', roles: ['admin'] })
